...
function buildEvent(attributes) {
  ...
  function formatDTEND(startString, endString, tz, tzEnd) {
    ...
    if (tz && !tzEnd && !endString) {
      return 'DTEND;TZID=' + tz + ':' + moment(startString).format('YYYYMMDDTHHmm00') + 'Z';
    }

    if (tz && !tzEnd && endString) {
      return 'DTEND;TZID=' + tz + ':' + moment(endString).format('YYYYMMDDTHHmm00') + 'Z';
    }

    if (tz && tzEnd && endString) {
      return 'DTEND;TZID=' + tzEnd + ':' + moment(endString).format('YYYYMMDDTHHmm00') + 'Z';
    }
    ...
    if (!endString && isDateTime(startString) && moment.parseZone(startString).utcOffset() === 0) {
      return 'DTEND:' + moment(startString).format('YYYYMMDDTHHmm00') + 'Z';
    }
  }
